...
public partial class _Default : System.Web.UI.Page
{
    ...
    private void dlvrdOrders()
    {
        string query = "Select * from cart_shop where cart_status='Delivered'";
        DataTable dtshpdOrders = getdlvrdorders(query);
        grdDeliveredOrder.DataSource = dtshpdOrders;
        grdDeliveredOrder.DataBind();
    }

    private DataTable getdlvrdorders(string query)
    {
        ...
        foreach (DataRow dr in cartP.Rows)
        {
            ...
            temp.Rows.Add(dr.ItemArray[0].ToString(), username, address, mobile, quantity, dr.ItemArray[9].ToString());
        }
    ...
    {
        string query = "Select * from cart_shop where cart_status='Shipped'";
    ...
    {
        string query = "Select * from cart_shop where cart_status='Pending'";
    ...
    private DataTable getshporders(string query)
    {
        ...
        temp.Columns.Add("user_s_address");
        temp.Columns.Add("mobile");
        temp.Columns.Add("shipped_date");
        temp.Columns.Add("ship_detail");
        temp.Columns.Add("total_items");
        temp.Columns.Add("cart_price");
        ...
        foreach (DataRow dr in cartP.Rows)
        {
            ...
            temp.Rows.Add(dr.ItemArray[0].ToString(), username, address, mobile,dr.ItemArray[7].ToString(), dr.ItemArray[8].ToString(), quantity, dr.ItemArray[9].ToString());
        }
    ...
    protected void btnPendingStats_Click(object sender, EventArgs e)
    {
        ...
        if(ddlstatuspending.SelectedValue.Equals("Shipped"))
        {
            UpdateQuery = "Update cart_shop set cart_shipped_date = '" + txtShipDate.Text + "',cart_status='" + ddlstatuspending.SelectedValue + "',cart_shipping_detail='" + txtShipDetails.Text + "' where cart_Id='" + hidValuCartId.Value + "'";
        }
        else
        {
    ...
    protected void grdDeliveredOrder_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        int index = Convert.ToInt32(e.CommandArgument);
        if (e.CommandName.Equals("show"))
        {
            string cartid = grdDeliveredOrder.Rows[index].Cells[0].Text;
            grdPendingDetView.DataSource = getgrdPendingDetView(cartid);
            grdPendingDetView.DataBind();
            mdlforPandingDetails.Show();
        }
